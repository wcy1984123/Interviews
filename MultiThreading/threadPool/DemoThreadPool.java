...
public class DemoThreadPool<Job extends Runnable> implements ThreadPool<Job> {
    ...
    private void init(int workNum) {
        // 初始化工作队列
        this.workQueue = new ArrayBlockingQueue<>(DEFAULT_WORKER_NUM);

        // 将指定数量的工作线程加入到列表中
        for (int i = 0; i < workNum; i++) {
            this.demoThreadList.add(new DemoThread(workQueue));
        }

        // 启动指定数量的工作线程
        for (DemoThread thread : demoThreadList) {
            Thread worker = new Thread(thread, "ThreadPool-Worker-" + this.threadNum.incrementAndGet());
            System.out.println("ThreadPool-Worker-" + this.threadNum.get() + " add to workQueue!");
            worker.start();
        }
    }
