 ...
 * However, you should prefer top-down instead of bottom-up in general, since
 ...
 * Please note that the algorithm requires the list’s length to be passed in as
 * the function’s parameters. The list’s length could be found in O(N) time by
 * traversing the entire list’s once. The recursive calls traverse the list and
 * create tree’s nodes by the list’s order, which also takes O(N) time.
