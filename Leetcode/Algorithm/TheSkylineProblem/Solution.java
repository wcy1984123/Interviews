...
public class Solution {
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        // sort edges
        // 按照 x 的坐标对 edge 进行排序
        // 如果 x 的坐标相等，而且如果两条 edge 都是 starting edge，则按照 height 长的排在前面
        // 如果两条 edge 都是 ending edge，则 height 低 的在前面
        // Else, starting edge 放在前面。
        Collections.sort(edges, new Comparator<Edge>() {
            public int compare(Edge a, Edge b) {
                if (a.x != b.x)
                    return Integer.compare(a.x, b.x);

                // for the case where a.x == b.x, starting edge with higher height would be put in the first position
        ...
        for (Edge edge : edges) {
            ...
            // using starting edge to form the coordinates for the problem
            if (edge.isStart) {
                if (heightHeap.isEmpty() || edge.height > heightHeap.peek()) {
                    result.add(new int[] { edge.x, edge.height });
                }
                heightHeap.add(edge.height);
            } else { // for the case of ending edge

                // remove the corresponding starting edge
